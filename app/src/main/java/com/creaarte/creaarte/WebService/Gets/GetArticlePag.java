...
public class GetArticlePag extends AsyncTask<String, String, List<ItemArts>> {
    ...
    public GetArticlePag(Activity activity, TextView textViewTitleArtsMenuMain, RecyclerView recyclerViewMainArtsMenuMain, NestedScrollView nestedScrollViewMenuMain) {
    ...
    protected List<ItemArts> doInBackground(String... params) {
        WebServiceClient client = new WebServiceClient(WebServiceClient.RequestMethod.POST, "getArticlePag");
    ...
    protected void onPostExecute(List<ItemArts> list) {
        ...
        if (code == 200) {
            CellItemArtsAdapter cellItemArtsAdapter = new CellItemArtsAdapter(activity, list);
            recyclerViewMainArtsMenuMain.setAdapter(cellItemArtsAdapter);
